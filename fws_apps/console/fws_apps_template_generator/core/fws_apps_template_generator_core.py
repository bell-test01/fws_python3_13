...
class FwsAppsTemplateGeneratorCore:
    ...
    def is_excluded(self, path_str: str) -> bool:
        ...
        for exclude in const.EXCLUDE_LIST:
            # プレースホルダーを置換
            resolved_exclude = exclude.replace("{app_name}", self.app_name)
    ...
    def generate_from_template(self) -> None:
        ...
        if self.template_type == "cli":
            template = const.CLI_TEMPLATE_STRUCTURE
        else:
            template = const.GUI_TEMPLATE_STRUCTURE

        print(f"Generating {self.template_type} template for {self.app_name} at {self.output_path}")

        for item in template:
            # プレースホルダーを置換
            rel_path = item.replace("{app_name}", self.app_name)
            
            if self.is_excluded(rel_path):
                continue

            full_path = self.output_path / rel_path
    ...
    def generate_from_structure_file(self, file_path: str) -> None:
        ...
        Returns:
            None - 戻り値なし。
        """
        structure_file = Path(file_path).resolve()
        ...
        for line in lines:
            ...
            try:
                ...
                if self.is_excluded(str(rel_from_root).replace('\\', '/')):
                    continue
            except ValueError:
                pass
